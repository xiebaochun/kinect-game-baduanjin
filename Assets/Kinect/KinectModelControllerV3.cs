	...
	void RotateJoint(int bone) {
		...
		if(bone % 4 == 0)
		{
			...
			if(bone == (int)Kinect.NuiSkeletonPositionIndex.HipLeft)
			{
				...
				target = ((sw.bonePos_2[player,(int)Kinect.NuiSkeletonPositionIndex.HipLeft] + sw.bonePos_2[player,(int)Kinect.NuiSkeletonPositionIndex.HipRight]) / 2F) - sw.bonePos_2[player,(int)Kinect.NuiSkeletonPositionIndex.HipCenter];
			}
			//otherwise it is one of the shoulders
			else
			{
				//target = vector from shoulder_center to bone
				target = sw.bonePos_2[player,bone] - sw.bonePos_2[player,(int)Kinect.NuiSkeletonPositionIndex.ShoulderCenter];
			}
		}
		else
		{
			//target = vector from previous bone to bone
			target = sw.bonePos_2[player,bone] - sw.bonePos_2[player,bone-1];
		}
		...
		if(bone == (int)Kinect.NuiSkeletonPositionIndex.Spine)
		{
			...
			dir = _chestRight;
			target = sw.bonePos_2[player,(int)Kinect.NuiSkeletonPositionIndex.ShoulderRight] - sw.bonePos_2[player,(int)Kinect.NuiSkeletonPositionIndex.ShoulderLeft];
		...
		else if(bone == (int)Kinect.NuiSkeletonPositionIndex.HipLeft)
		{
			...
			dir = _hipRight;
			target = sw.bonePos_2[player,(int)Kinect.NuiSkeletonPositionIndex.HipRight] - sw.bonePos_2[player,(int)Kinect.NuiSkeletonPositionIndex.HipLeft];
